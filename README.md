...
| チャレンジ記録 | 右下の「▶ きろく」で切り替え。成功・失敗をGoogleスプレッドシートに記録します（[下記](#チャレンジ記録)） |
...
v0.8.0 で追加した記録画面です。画面右下の「▶ きろく」で容量管理と切り替えます。

- 「成功で記録」「失敗で記録」で、寝かせ時間（端末の稼働時間から自動入力）・使用サービス（LINE / Google）・メモをスプレッドシートに追加します
- 送り先は、書き込み専用に用意した Google Apps Script のウェブアプリです。共通パスワードで認証するので、端末でGoogleにログインする必要はありません
- 通信できなかった記録は端末に「未送信」として残り、あとで自動または手動で送り直せます。同じ記録が二重に登録されることはありません
- 初回に記録APIのURL・共通パスワード・端末IDを入力します。URLとパスワードは、`scripts/make_setup_qr.py` で作ったQRコードをカメラで読み取ってコピーし、「クリップボードから読み込む」で入力できます

> [!NOTE]
> 記録先のスプレッドシートと Apps Script はこのリポジトリに含まれていません。URL・パスワード・端末IDはアプリにもリポジトリにも埋め込まず、端末ごとに入力する方式です。記録画面を使わなければ、容量埋めの機能はこれまでどおり単体で動きます。
